...
if (function_exists('add_theme_support'))
{
    ...
    add_image_size('small_height', '', '400', true); // Small Thumbnail
...
function webfactor_header_scripts()
{
    if ($GLOBALS['pagenow'] != 'wp-login.php' && !is_admin()) {
        ...
        // wp_register_script('matchHeight', get_template_directory_uri() . '/js/lib/jquery.matchHeight.js', array(), '2.7.1'); // Modernizr
...
function chilly_nav($menu){
    ...
    foreach ($items as $item ) {
        $link = get_page_link($item->object_id);
        $str .= '<li><a title="'. $item->title  .'" href="'. $link  .'">'. $item->title  .'</a></li>' . "\n";
    }
    echo $str;
}
...
function get_event_chevron_classes($index) {

    $classes = array();


    if ($index % 6 > 2) {
        array_push($classes, 'se_md_flip');
    }
    if ($index % 3 == 0) {
        array_push($classes, 'se_md_push_right');
    }
    if ($index % 3 == 2) {
        array_push($classes, 'se_md_push_left');
    }


    if ($index % 4 > 1) {
        array_push($classes, 'se_sm_flip');
    }

    if ($index % 2 == 0) {
        array_push($classes, 'se_sm_push_right');
    }
    if ($index % 2 == 1) {
        array_push($classes, 'se_sm_push_left');
    }

    if ($index % 2 == 1) {
        array_push($classes, 'se_xs_flip');
    }
...
include('functions_posttypes.php');
